...
function drop(ev) {
    if($ (ev.target).attr('id')=="div1"){
      ...
      $.post('http://localhost/ControlUVFEI/index.php/welcome/web_service', {"salon":id_salon,"hora":hora,"dia":dia,"nrc":data}, function(result){
        console.log(result);
      }).fail(function () {
        alert('Error AJAX');
      });
...
<div id="curso" class="col-sm-2"  id="div1" ondrop="drop(event)" ondragover="allowDrop(event)">
  <?php 
    $query =$this->db->get('asignatura');
      foreach ($query ->result() as $key => $row) {
        echo '<div id="'.$row->IDA,'" draggable="true" ondragstart="drag(event)">'.$row->Asignatura.'</div>';
      } 
  ?>
</div>
<div class="col-sm-1">
